...
- [AI Trading Bot](../projects/trading-bot.md): 별도 비공개 레포에서 운영하고, 공개 가능한 작업 로그와 기술적 의사결정만 포트폴리오에 반영하는 자동매매봇 실험.
- [WebGPU Game](../projects/webgpu-game.md): AI를 활용해 WebGPU 기반 게임을 만드는 장기 실험 프로젝트.

## Prompt Library Pages

- [Project Planning Prompt](../prompts/project-planning.md): 구현 전에 목적, 범위, 콘텐츠 구조, 작업 우선순위를 잡는 프롬프트.
- [Documentation Prompt](../prompts/documentation.md): AI와 함께 만든 결정, 작업 과정, 검증 결과를 문서로 남기는 프롬프트.
- [Daily Work Log Prompt](../prompts/daily-work-log.md): Build Log와 Daily Digest를 공유 가능한 작업 기록으로 묶는 프롬프트.
- [Code Review Prompt](../prompts/code-review.md): 변경 사항을 리뷰 관점에서 점검하는 프롬프트.
- [Multi-Agent Scope Handoff Prompt](../prompts/multi-agent-scope-handoff.md): 여러 에이전트가 같은 워크스페이스에서 일할 때 담당 범위를 분리하는 프롬프트.
- [Vercel Mock Production Deploy Report Prompt](../prompts/vercel-mock-production-deploy-report.md): mock-only Production 배포 결과를 기록하는 프롬프트.

## AI Stack Pages

- [ChatGPT](../ai-stack/chatgpt.md): 초기 기획, 정보 구조 설계, 문서 초안 작성에 사용하는 AI 도구.
- [Codex](../ai-stack/codex.md): 현재 레포 상태를 읽고 구현, 검증, 문서화를 이어가는 개발 파트너.
- [Claude](../ai-stack/claude.md): 긴 문서 정리, 설계 비교, 코드 리뷰, 표현 다듬기에 사용하는 AI 도구.
...
- [Vercel Connection Decision Brief](./vercel-connection-decision-brief.md): Vercel 연결을 보류할지, mock-only Production으로 갈지, Preview real mode까지 준비할지 결정하는 문서.
- [Vercel Mock-only Production Runbook](./vercel-mock-production-runbook.md): Option B 선택 시 Production은 mock mode로만 배포하는 실행 절차.
...
- `knowledge/VERCEL_CONNECTION_DECISION_BRIEF.md`: Vercel 연결 여부 결정 브리프.
- `knowledge/VERCEL_MOCK_PRODUCTION_RUNBOOK.md`: mock-only Production 배포 절차.
- `knowledge/VERCEL_MOCK_PRODUCTION_DEPLOY_REPORT_TEMPLATE.md`: mock-only Production 배포 결과 기록 템플릿.
