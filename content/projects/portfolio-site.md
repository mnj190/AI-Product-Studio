...
Vercel mock-only Production 배포까지 완료했다.

공개 URL은 `https://ai-product-studio-psi.vercel.app/`이며, `/api/ask`는 `mode=mock`으로 동작한다.

real LLM mode는 Production에 켜지 않고, 이후 Vercel Preview에서만 별도로 검수한다.
