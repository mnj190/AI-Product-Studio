...
- npm registry 기준 latest `next@14`는 `14.2.35`이고 현재 lockfile도 `next@14.2.35`를 사용하므로 14 라인 안에서 더 올릴 패치 버전은 없다.
- latest `next@15`는 `15.5.19`, latest `next`는 `16.2.9`이다.
...
- `knowledge/NEXT_ACTIONS.md`의 audit 검토와 보안 패치 경로 확인 항목을 완료 처리하고, 배포 전 Next major upgrade 여부 결정을 남겼다.
...
- Vercel Option B/C를 선택하기 전에 Next 15/16 업그레이드 검증 여부를 결정한다.
